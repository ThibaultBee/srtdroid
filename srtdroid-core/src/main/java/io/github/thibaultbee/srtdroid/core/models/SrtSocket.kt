...
private constructor(private val srtsocket: Int) : ConfigurableSrtSocket, Closeable {
    ...
    constructor(af: StandardProtocolFamily) : this(
        nativeCreateSocket(
            af,
            0,
            0
     ...
     * @see [recv]
     */
    fun send(msg: ByteArray, msgCtrl: MsgCtrl) =
        send(msg, 0, msg.size, msgCtrl)
     ...
     * @see [recv]
     */
    fun send(msg: String, msgCtrl: MsgCtrl) =
        send(msg.toByteArray(), msgCtrl)
    ...
    fun getOutputStream(msgCtrl: MsgCtrl? = null) =
        SrtSocketOutputStream(
            this,
            msgCtrl
        ) as OutputStream

    private class SrtSocketOutputStream(
        private val socket: SrtSocket,
        private val msgCtrl: MsgCtrl?
    ) :
        OutputStream() {
    ...
    fun getInputStream(msgCtrl: MsgCtrl? = null) =
        SrtSocketInputStream(
            this,
            msgCtrl
        ) as InputStream

    private class SrtSocketInputStream(
        private val socket: SrtSocket,
        private val msgCtrl: MsgCtrl?
    ) :
        InputStream() {

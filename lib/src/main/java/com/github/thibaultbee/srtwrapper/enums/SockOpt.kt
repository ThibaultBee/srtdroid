...
enum class SockOpt {
    ...
    CONNTIMEO,       // Connect timeout in msec. Caller default: 3000, rendezvous (x 10)
    DRIFTTRACER,     // Enable or disable drift tracer
    ...
    BINDTODEVICE,    // Forward the SOL_SOCKET/SO_BINDTODEVICE option on socket (pass packets only from that device)
    PACKETFILTER,    // Add and configure a packet filter
    RETRANSMITALGO   // An option to select packet retransmission algorithm
}